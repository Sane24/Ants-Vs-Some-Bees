...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        throw=self.place
        count=0
        while not throw.is_hive:
            if (self.upper_bound==None and self.lower_bound==None) or (self.upper_bound!=None and self.lower_bound<=count<=self.upper_bound) or (self.upper_bound==None and count>=self.lower_bound):
                if throw.bees:
                    return random_bee(throw.bees)
            throw=throw.entrance
            count+=1
        return None
        '''
        throw=self.place
        while not throw.is_hive:
            if throw.bees: #self.place.bees (Double dot)
                return random_bee(throw.bees)
            throw=throw.entrance
        return None
        '''
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.place.bees and self.turns_to_chew<=0:
            ...
            self.turns_to_chew=HungryAnt.chewing_turns + 1
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while dub:
            if dub.ant:
                if dub.ant.is_container and dub.ant.ant_contained:
                    dub.ant.ant_contained.double()
                dub.ant.double()
            dub=dub.exit
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        target.sticky= 0
        def whyy(gamestate):
            if target.sticky>=5: #when bee is not sticky
                beee(target,gamestate)
            elif gamestate.time%2==0: #when bee is sticky and time is even
                beee(target,gamestate)
            target.sticky=target.sticky+1
        ...
        def new_action(gamestate):
            ...
            if turns>0:
                ...
                if gamestate.time%2==0:
                    old(gamestate) #when bee is sticky and time is even
            else:
                old(gamestate) #when bee is not sticky
